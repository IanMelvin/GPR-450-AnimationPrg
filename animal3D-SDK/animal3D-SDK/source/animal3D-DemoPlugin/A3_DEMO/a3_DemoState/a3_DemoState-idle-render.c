	...
	a3f32 const textAlign, a3f32 const textDepth, a3f32 const textOffsetDelta, a3f32 textOffset)
{
	...
	if (demoState->testAnimator.clipPool) sprintf_s(buf, 256, "Clip[%i] '%s': %f at time=%f", demoState->testAnimator.clipIndex, ec_clipController_getClip(&demoState->testAnimator)->name, evaluatedValue, demoState->testAnimator.clipTime);
	else strcpy_s(buf, 256, "[Error: Test animator was not initialized]");
	a3textDraw(text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a, buf);
